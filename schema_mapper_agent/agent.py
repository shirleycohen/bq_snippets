...
def get_table_listing(project: str, source_dataset: str, target_dataset: str, target_schema: str) -> dict:
    ...
    Args:
        ...
        source_dataset (str): The name of the dataset in BigQuery where the source tables are stored
        target_dataset (str): The name of the dataset in BigQuery where the target tables will be stored
        target_schema (str): The name of the target schema in BigQuery which the source tables will be mapped to

    Returns:
        dict: A dictionary containing the status, source dataset, target dataset, list of source tables, and target schema.
    """
    ...
    return {"status": success, "source_dataset": source_dataset, "target_dataset": target_dataset, "source_tables": source_tables, "target_schema": target_schema}


def generate_prompts(source_dataset: str, target_dataset: str, source_tables: list, target_schema: str) -> dict:
    ...
    Args:
        source_dataset (str): The name of the dataset from where the tables will be sourced. 
        target_dataset (str): The name of the dataset where the target table resides. 
        source_tables (list): The list of table names that we want to source from.
        target_schema (str): The name of the target schema that will be mapped to. 

    Returns:
        dict: A dictionary containing the status and list of generated prompts. 
    """
    ...
    for source_table in source_tables:
        ...
        prompt = f"""Given the source table, {PROJECT}.{source_dataset}.{source_table}, and the target schema, 
                    {PROJECT}.{target_dataset}.{target_schema}, generate a Dataform pipeline that transforms 
                    the data from the source table to the target schema. Store the results as a new table,
                    {PROJECT}.{target_dataset}.{source_table}_transformed."""

        prompts.append(prompt)
        print(f"generated prompt for {source_dataset}.{source_table}")
...
def merge_output_tables(project: str, target_dataset: str, target_schema: str, final_table: str) -> dict:
    
    """Merges the output tables to produce the final output

    Args:
        project (str): The project id of the GCP project where the tables are stored. 
        target_dataset (str): The name of the dataset in BigQuery where the target tables will be stored
        target_schema (str): The name of the target schema in BigQuery which the source tables will be mapped to
        final_table (str): The name of the final table that will be created from the merge operation

    Returns:
        dict: A dictionary containing the status of the merge operation and the number of rows merged
    """

    success = True
  
    client = bigquery.Client()
    dataset_id = f"{project}.{target_dataset}"

    print(f"Fetching the output tables from the dataset '{dataset_id}':")

    try:
        tables = client.list_tables(dataset_id)
      
        merge_stmt = f"create or replace table `{project}.{target_dataset}.{final_table}` as "
        counter = 0
      
        for table in tables:
            
            if table.table_id == target_schema:
                continue
            ...
            if counter > 1: 
                merge_stmt += " union distinct "
            
            merge_stmt += f"select * from `{project}.{target_dataset}.{table.table_id}`"
            
        print(f"Info: merge_stmt: {merge_stmt}")
        job = client.query(merge_stmt)
        job.result()
        
        query = f"select count(*) from `{project}.{target_dataset}.{final_table}`"
        results = client.query(query)
        
        for row in results:
            row_count = row[0]
            print(f"Info: number of rows merged: {row_count}")

    except Exception as e:
        print(f"Error occurred while merging the output tables in {target_dataset}: {e}")
        success = False

    return {"status": success, "num_rows": row_count}
...
root_agent = Agent(
    ...
    instruction=(
        ...
        calling the Data Engineering Agent for each prompt, running the Dataform pipeline, and finally merging the output into a final table. 
        ...
        Return the status and other outputs from each step of the workflow and check with the user before continuing to the next step. 
        If the user tells you to proceed with calling the Data Engineering Agent, please invoke the tool for each of the tables 
        without asking the user to consent to each invocation.  
        """
    ),
    tools=[get_table_listing, generate_prompts, initialize_dataform_workspace, call_data_engineering_agent, execute_dataform_pipeline, merge_output_tables],
)